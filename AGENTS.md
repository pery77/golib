...
| Games in the browser: a web backend of GoLib's own, on WebGL 2 and Web Audio, and `golib web` | Done for the picture, the input and the sound (2026-09-18, stages 0 to 2): `games/platformer`, `crates` and `tetris` play in a browser, with their sound effects and music. No post-processing shaders, fonts from files, `.xm`, `.mod` or `.qoa`, or saving between visits yet, and nobody has heard a web build; the stages are in [docs/roadmap.md](docs/roadmap.md#web-build-started-2026-09-18) |
...
| `web [game] [--port <n>] [--no-open]` | Builds the game for the browser into `build/<game>/web/` and serves it at a `http://localhost` address, opening it unless `--no-open`. The picture, the input and the sound work; post-processing shaders, fonts from files, `.xm`, `.mod`, `.qoa` and saving between visits don't yet (see [docs/tooling.md](docs/tooling.md#web-builds)). |
