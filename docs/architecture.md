...
> **Status:** the framework and game split exists since M1, with `framework/` and a first game, `games/hello`. Loading content from Tiled, Aseprite and Blender is planned for M2 (see [roadmap.md](roadmap.md)).
...
games/
  <name>/           One game; the folder name is the game's short name
    ...
    assets/         Content: maps, sprites, models, sounds, fonts (from M2)
```
...
The framework is built together with a real game that tries out each feature as it lands; today that is `games/hello`. It follows the same rules as any other game: it is the framework's first user, not part of it. When it needs something the framework lacks:
...
| --- | --- | --- | --- |
| 2D maps and levels | [Tiled](https://www.mapeditor.org) | Tiled maps and tilesets | M2 |
| Sprites and animations | [Aseprite](https://www.aseprite.org) | Aseprite sprites, with their animation tags | M2 |
| 3D models | [Blender](https://www.blender.org) | glTF (`.glb`) exported from Blender | With 3D support |
| Sound effects and music | Any audio tool | `.wav`, `.ogg` | M2 |
| Fonts | Existing fonts whose license allows it | `.ttf` | M2 |
