...
- To do: the notices of the libraries built into raylib that are under the MIT license, which raylib's `LICENSE` doesn't cover: cgltf, tinyobj_loader_c, vox_loader, m3d, par_shapes, qoi, qoa and glad's Khronos code. Their texts are in their headers, in raylib-go's `external/` folder; `THIRD-PARTY-LICENSES.txt` could take them from there, or from a file committed in `tools/cli`, once someone checks which of them the prebuilt library contains. The others (GLFW, miniaudio, stb, dr_libs, jar_xm, jar_mod, sinfl, sdefl, rprand) are zlib, public domain or MIT-0, which ask for nothing in programs.
...
- `THIRD-PARTY-LICENSES.txt` lacks the notices of the MIT-licensed libraries inside raylib (see the to-do above). Until it has them, whoever publishes a game should add them by hand.
