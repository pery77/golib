...
- In progress: the CLI in Go. The command logic moves from the twin scripts into one Go program, `tools/cli`, one command at a time (see [Decisions](#decisions) and [tooling.md](tooling.md#the-go-program)).
  - Done on 2026-09-17: `dist`, with `tools/shipping` folded in. The scripts build the program into `build/golib/` and start it with the user's environment, and it sets GoLib's environment for the `go` commands it runs. It now checks `game.json` and `icon.png` on Linux and macOS too, and prints a game's title correctly when it isn't ASCII. On Windows, its executables match the ones the script made, byte for byte, apart from Go's build information.
  - To do: `new`, `build`, `run`, `shot` and `test`, then the parts of `setup` and `doctor` that need Go, such as downloading modules and extracting raylib.
- To do, now that `dist` is in Go: dist builds as a folder, `build/<game>/dist/<game>/`, with the executable, raylib and libffi side by side, and a `THIRD-PARTY-LICENSES.txt` with the notices of the third-party code built into the game (Go, purego, raylib-go, ffi, raylib, libffi), plus a zip of that folder to share, instead of one executable that writes the libraries into the player's cache folder (see [Decisions](#decisions)).
...
- Dist builds carry no third-party notices, although the licenses of some code built into them ask binaries to include one: Go (BSD-3-Clause), purego (Apache-2.0), ffi and libffi (MIT). Until the folder dist build adds `THIRD-PARTY-LICENSES.txt`, whoever publishes a game should include those notices by hand. raylib and raylib-go (zlib) ask for nothing in binaries.
